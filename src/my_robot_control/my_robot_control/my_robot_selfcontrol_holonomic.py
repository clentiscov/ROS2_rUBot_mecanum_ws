...
class RobotSelfControl(Node):
    ...
    def laser_callback(self, scan):
        ...
        # React to obstacle
                # React to obstacle (versión holonómica usando también movimiento lateral)
        if closest_distance < self._distanceLimit:
            if zone == "FRONT":
                # Obstáculo delante -> me aparto lateralmente hacia la izquierda
                ...
                # Fuera de FOV útil -> comportamiento por defecto
                self._msg.linear.x = self._forwardSpeed * self._speedFactor
